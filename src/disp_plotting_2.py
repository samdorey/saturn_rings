...
for y in np.linspace(15,30,num=len(axes)):
    ...
    axis.set_xlabel("Time/arbitrary units")
    axis.set_ylabel("Object Displacement/arbitrary units")
    axis.set_ylim(top=160, bottom=0)
    axis.set_xlim(left=0,right=out.t.max())
    axis.plot(out.t,sun_displacement[0,:].T,'k--', label="Galaxy 2", color="black", linewidth="1.5" )
    axis.legend()
